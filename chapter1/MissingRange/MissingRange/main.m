...
NSArray *findMissingRange(NSArray *intArray){
    ...
    if (![intArray[0]isEqualToNumber:@0]) {
        if ([intArray[1] isEqualToNumber:@1]) {
            [missingRange addObject:@"0"];
        }else{
            [missingRange addObject:[NSString stringWithFormat:@"0->%d",[intArray[1] intValue]-1]];
        }
    }
    
    
    for (NSUInteger i=2; i<intArray.count; i++) {
    ...
    if (![[intArray lastObject] isEqualToNumber:@99]) {
        if ([[intArray lastObject] isEqualToNumber:@98]) {
            ...
            [missingRange addObject:[NSString stringWithFormat:@"%d->99",[intArray[count-2] intValue] + 1]];
        }
    }
